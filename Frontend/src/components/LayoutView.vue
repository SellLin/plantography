<template>
  <div style="height: 100%;">
    <!--    buttons-->
    <div style="position: absolute; top: 29%; left: 0; width: 50px;background: #efefe8">
      <div
        ...
        <button class="svg-button-1" @click="open()">
          <svg id="arrow" viewBox="0 0 41.72 32.14" transform="translate(4.1 0)">
        ...
        <button class="svg-button-1" @click="drawForce(layoutData)">
          <svg id="refresh" viewBox="0 0 44.72 32.14">
            <rect class="cls-1" width="29.33" height="29.33" rx="6" ry="6"/>&ndash;&gt;-->
            <path class="cls-2"
    ...
    <!--    graph -->
    <div
        style="background: #ffffff; float: right; position:absolute; top: 5%; right: 9px; width: calc(100% - 50px); border-radius: 18px">
      <div>
        <div
            style="height: 30px; font-family: Montserrat-Bold,serif; font-size:20px; font-weight:600; color: rgb(97 108 24); margin-left: 25px; margin-top: 13px; text-align: start;">
          Layout
        </div>


        <!--  main  graph-->
        <div style="background: #ffffff; height:52vh; border-radius: 18px">
          <svg ref="layoutView" style="width: 100%; height: 100%" id="layoutBox"></svg>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    drawForce(layout_data) {
      ...
      let formatted_data = layout_data.map(d => ({name: d[0], coords: d[1]}));
      ...
      function draged(event){
         if (thi.scaleState==0) {
           d3.select(this).attr("x", event.x - dragStartX + Number(d3.select(this).attr("startX"))).attr("y", event.y - dragStartY + Number(d3.select(this).attr("startY")))
           // d3.select(this).attr("x",event.x).attr("y",event.y)
           d3.select("#rectText" + d3.select(this).attr("rect_id")).attr("x", 10 + event.x - dragStartX + Number(d3.select(this).attr("startX")))
               .attr("y", 20 + event.y - dragStartY + Number(d3.select(this).attr("startY")))
         }
         else{
           if (event.x - dragStartX>0){
             d3.select(this).attr("width", Number(d3.select(this).attr("width"))+(event.x - thi.prevX))
           }
           else{
             d3.select(this).attr("width", Number(d3.select(this).attr("width"))+(event.x - thi.prevX))
           ...
           if (event.y - dragStartY<0){
             d3.select(this).attr("height", Number(d3.select(this).attr("height"))+(event.y - thi.prevY))
           }
           else{
             d3.select(this).attr("height", Number(d3.select(this).attr("height"))+(event.y - thi.prevY))
    ...
    changeLayout(){
      ...
      for (let i=0; i<allRects._groups[0].length; i++){
        ...
        let x=Number((allRects._groups[0][i]).attributes["x"]["value"])-this.translateX-10
        let y=Number((allRects._groups[0][i]).attributes["y"]["value"])-this.translateY-20
        let width=Number((allRects._groups[0][i]).attributes["width"]["value"])
        let height=Number((allRects._groups[0][i]).attributes["height"]["value"])
        let name=(allRects._groups[0][i]).attributes["name"]["value"]
        layoutList.push({"name": name, "layout": [x, y, width, height]})
        layoutStr=layoutStr+name+","+x+","+y+","+width+","+height+";"
      }
    ...
    addBox(name){
      ...
      function draged(event){
         if (thi.scaleState1==0) {
           d3.select(this).attr("x", event.x - dragStartX1 + Number(d3.select(this).attr("startX"))).attr("y", event.y - dragStartY1 + Number(d3.select(this).attr("startY")))
           // d3.select(this).attr("x",event.x).attr("y",event.y)
           d3.select("#rectText" + d3.select(this).attr("rect_id")).attr("x", 10 + event.x - dragStartX1 + Number(d3.select(this).attr("startX")))
               .attr("y", 20 + event.y - dragStartY1 + Number(d3.select(this).attr("startY")))
         }
         else{
           if (event.x - dragStartX1>0){
             d3.select(this).attr("width", Number(d3.select(this).attr("width"))+(event.x - thi.prevX1))
           }
           else{
             d3.select(this).attr("width", Number(d3.select(this).attr("width"))+(event.x - thi.prevX1))
             // d3.select(this).attr("x", Number(d3.select(this).attr("x"))+event.x - dragStartX)
           ...
           if (event.y - dragStartY1<0){
             d3.select(this).attr("height", Number(d3.select(this).attr("height"))+(event.y - thi.prevY1))
           }
           else{
             d3.select(this).attr("height", Number(d3.select(this).attr("height"))+(event.y - thi.prevY1))
             // d3.select(this).attr("y", Number(d3.select(this).attr("y"))+event.y - dragStartY)
