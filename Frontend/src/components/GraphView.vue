<template>
  <div style="height: 100%;">
    <!--    buttons-->
    <div style="position: absolute; top: 27%; left: 0; width: 50px; background: none">
      <div
          style="background: none; display: flex;flex-direction: column;align-content: flex-start;justify-content: center;align-items: flex-start;">
        <!--       Graph Buttons -->
        <!--              <el-row>-->
        <button class="svg-button" @click="open()">
          <svg id="arrow" transform="translate(4.1 0)" viewBox="0 0 41.72 32.14">
    ...
    <!--    graph -->
    <div class="view" id="graph_real"
         style="background: #ffffff; float: right; position:absolute; top: 5%; right: 3px; width: calc(100% - 50px);overflow: visible">
      <div style="float:left;margin-left:-80px">
        <div
            style="height: 30px; font-family: Montserrat-Bold,serif; font-size:20px; font-weight:600; color: rgb(97 108 24); margin-left: 95px; margin-top: 13px; text-align: start">
          Graph
        </div>
        <br>
        ...
        <!--  main  graph-->
        <div style="background: #ffffff; height: 36vh; width:520px; border-radius: 18px;display: inline-block;margin-left:0px;position:absolute">
...
<!--  <el-checkbox-group v-model="nodesData" >-->
          <div v-for="(node, index) in nodesData" style="display:inline-block">
...
<!--            <circle r="20px" :style="'fill: '+this.colors[index]"></circle>-->
            <div :id="'node_span'+index" :style="{ backgroundColor: colors(index), display: 'inline-block', marginLeft: '8px',opacity: 0.5, fontSize: '7px', borderRadius: '2px' }"><span style="opacity: 1">{{node.name}}</span></div>
            <el-input style="width:60px;margin-left:1px" :id="'rel'+index" v-model="relInput[index]" size="small" placeholder="Relation" ></el-input>
            </div>
            <el-select v-model="addType" placeholder="Select Add" style="display:none">
            <el-option
...
<!--          <el-button @click="addNewNode">Add New Node</el-button>-->
          <el-button @click="realAddNode(graphData)" style="margin-left:8px">{{'Add '+addType}}</el-button>
        </div>

          <svg ref="sceneGraph" style="width: 100%; height: 100%; float:left; overflow: visible"></svg>


        </div>
        <div id="graph-gallery" style="display: inline-block;position:absolute; height:36vh; width:220px;margin-left:500px;background:#ffffff;">
<!--         <h1>hello</h1>-->
          <div
            style="height: 30px; font-family: Montserrat-Bold,serif; font-size:16px; font-weight:600; color: rgb(97 108 24); margin-top: 13px; text-align: center;">
          Plants
        </div>

          <div class="img_row" v-for="(duo, index) in img_list">
            <img class="graph_img" :graph_id="2*index" :src="'./src/assets/addOptions/'+img_list[index][0]+'.png'" width="75" height="75" :plant="img_list[index][0]" v-on:dblclick="addClick">
            <img class="graph_img" :graph_id="2*index+1" :src="'./src/assets/addOptions/'+img_list[index][1]+'.png'" width="75" height="75" :plant="img_list[index][1]" style="margin-left:12px" v-on:dblclick="addClick">
...
export default {
  data() {
    return {
      ...
      nodesAddCon : [],
      addType: '',
      addOptions: [{value: "Daisy",label: "Daisy"},
      ...
      relInput: ["", "","", "", "", "", "", "", "", "","",""],
      colors: null,
      img_list: [["Africanlily","bougainvillea"], ["Crape_Myrtle", "Daisy"], ["Dogwood", "Japanese_Pine"], ["Maple", "white_tulip"], ["Weeping_Willow", "Blue_Jacaranda"]],
    }
  ...
  props: {
    objs: {
      type: Array,
      default: () => ["bougainvillea", "poinciane", "dogwood", "bush", "alumroot"]
    },
    triples: {
      type: Array,
      default: () => [
...
//           });
      ...
      svg.append("g")
          ...
          .attr("cx", d => d.x)
          .attr("cy", d => d.y)
          .attr("fill", () => `hsl(${20 + Math.random() * 40}, ${Math.random() * 20 + 70}%, ${Math.random() * 10 + 60}%)`) // random warm color
    ...
    drawForce(scene_graph_data) {
      ...
      let gWapper = svg.append('g')
          ...
          .attr('cursor', 'pointer')
      //绘制连线
      this.links = gWapper.append('g') //root
          .selectAll('line')  //dom
      ...
      this.linkRects = gWapper.append('g') //root
          .selectAll('line')  //dom
          .data(linksData)  //model
          .enter()
          .append('rect')
      ...
      let texts = gWapper.append('g')
          ...
          .text(d => d.name)
          .call(this.drag)
      let linkTexts = gWapper.append('g')
          .selectAll('.linktext')
          .data(linksData)
          .enter()
          .append('text')
    ...
    realAddNode(graphData){
      ...
      for (let i=0; i<this.nodesData.length;i++){
        ...
        if (this.relInput[i]!=""){
          let newLinks = [{source: i, target: this.nodesData.length, relationship: this.relInput[i]}]
  ...
  if (nodeName) {
     if (!this.nodesData1 || this.nodesData1.length === 0) {
      ...
      this.nodesData1.push({ name: nodeName, id: lastID + 1, target: targetName });
    }
  }
  ...
  this.targetName = ''; // Reset the input
    ...
    addNode() {
      //动态增加节点
      let newNodes = [{name: "河北", id: 5}, {name: "青岛", id: 6}]
      let newLinks = [{source: 5, target: 0}, {source: 6, target: 1}]
    ...
    ticked() {
      ...
      this.linkRects
          .attr('x', d=>((d.source.x+d.target.x)/2)-d.relationship.length*3)
