...
def run(
    ...
    with torch.autocast("cuda", enabled=use_autocast):
        ...
        print('over all prompt:', overall_prompt)
...
def run_in_batch(
    ...
    ########################################################################
    # The batch generation starts from here!
        for index in range(0, batch_size):
            # initialize the input latent for both bg and foreground instances
            input_latents_list, latents_bg = latents.get_input_latents_list(
                model_dict,
                bg_seed=bg_seed+index,
                fg_seed_start=fg_seed_start,
                so_boxes=so_boxes,
                fg_blending_ratio=fg_blending_ratio,
                height=height,
