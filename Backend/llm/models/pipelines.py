...
def generate_gligen(model_dict, latents, input_embeddings, num_inference_steps, bboxes, phrases, num_images_per_prompt=1, gligen_scheduled_sampling_beta: float = 0.3, guidance_scale=7.5, 
    ...
    dynamic_num_inference_steps=False, fast_after_steps=None, fast_rate=2, generate_index=0):
    """
    ...
    print("num_grounding_steps" , num_grounding_steps)
    ...
    img_save = Image.fromarray(images[0])
    img_save.save("batch_result\crepen_tulip\image_crepen_tulip_{index}.png".format(index=generate_index))
